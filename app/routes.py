...
def login():
    ...
    if form.validate_on_submit():
        # form.validate_on_submit() is True when method if type POST,
        # and all valid forms are filled
...
def index():
    ...
    posts = current_user.followed_posts().paginate(
        page, app_var.config['POSTS_PER_PAGE'], False)
...
@app_var.route('/follow/<username>', methods=['POST'])
@login_required
def follow(username):
    form = EmptyForm()
    if form.validate_on_submit():
...
@app_var.route('/unfollow/<username>', methods=['POST'])
@login_required
